<script lang="ts">
  ...
  async function movePlayerToSession() {
    ...
    try {
      ...
      if (!playerToMove) {
        error = m.move_player_to_session_error_not_found();
        return;
      }
...
<Modal title={m.move_player_to_session_title()} bind:open={open} size="md"
       autoclose={false}>
  <div class="space-y-4">
    {#if error}
    ...
    <div>
      ...
      <Select id="player-select" bind:value={selectedPlayerId} class="w-full">
        <option value={null} disabled
                selected>{m.move_player_to_session_select_player_placeholder()}</option>
    ...
    <div>
      ...
      {#if isLoading}
        <p
          class="text-sm text-gray-500">{m.move_player_to_session_loading()}</p>
      {:else if availableSessions.length === 0}
        <p
          class="text-sm text-gray-500">{m.move_player_to_session_no_sessions()}</p>
      {:else}
        <Select id="session-select" bind:value={selectedSessionId}
                class="w-full" placeholder="">
          <option value={null} disabled
                  selected>{m.move_player_to_session_select_session_placeholder()}</option>
          {#each availableSessions as session}
            <option value={session.id}>{session.name} (ID: {session.id})
            </option>
          {/each}
        </Select>
      {/if}
    ...
    <div class="flex justify-end space-x-2 pt-4">
      <Button type="button" color="light"
              onclick={closeModal}>{m.move_player_to_session_cancel()}</Button>
      <Button
        type="button"
        color="blue"
