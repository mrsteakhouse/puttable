<script lang="ts">
  ...
  import AddPlayerToSessionModal
    from "$lib/components/AddPlayerToSessionModal.svelte";
  import MovePlayerToSessionModal
    from "$lib/components/MovePlayerToSessionModal.svelte";
  import RemovePlayerFromSessionModal
    from "$lib/components/RemovePlayerFromSessionModal.svelte";
  ...
  let totalScore = $derived(scorecards.map(sc => sc.data.reduce((a, b) => a + b, 0)));
  let onesCount = $derived(scorecards.map(sc => sc.data.filter(score => score === 1).length));
  let playedHolesCount = $derived(scorecards.map(sc => sc.data.filter(score => score > 0).length));
  ...
  onMount(async () => {
    ...
    subscription = supabase
      ...
      .on("postgres_changes", {
          ...
          console.log("Updated scorecard:", updatedScorecard, "Scorecard index:", scorecardIndex);
          if (scorecardIndex !== -1) {
  ...
  const debouncedUpdateDatabase = debounce(async () => {
    ...
    const latestChanges = new Map<string, {
      recordId: number,
      arrayIndex: number,
      newValue: number
    }>();
    ...
    const changesByRecordId = {} as Record<number, {
      arrayIndex: number,
      newValue: number
    }[]>;
...
<div class="max-w-2xl mx-auto p-4 dark:bg-gray-800 dark:rounded-lg">
  ...
  <Tabs tabStyle="underline" ulClass="flex-wrap">
    <TabItem open={true} title={m.session_overview()}>
      ...
      <div class="mt-4 dark:text-gray-300">
        ...
        <div
          class="hidden md:grid md:grid-cols-6 md:gap-2 font-medium dark:text-white mb-2">
        ...
        {#each scorecards as sc, i}
          ...
          <div
            class="md:hidden bg-gray-50 dark:bg-gray-700 p-3 rounded-lg mb-3">
            <div class="font-medium dark:text-white mb-1">
              <a href={`/player/${sc.playerId}`}
                 class="text-blue-600 dark:text-blue-400 hover:underline">{sc.playerName}</a>
            </div>
            <div class="grid grid-cols-3 gap-2 text-sm">
              <div>
                <span
                  class="font-medium dark:text-white">{m.session_score_label()}</span> {totalScore[i]}
              </div>
              <div>
                <span
                  class="font-medium dark:text-white">{m.session_ones_label()}</span> {onesCount[i]}
              </div>
              <div>
                <span
                  class="font-medium dark:text-white">{m.session_holes_label()}</span> {playedHolesCount[i]}
              </div>
            </div>
          </div>
          ...
          <div
            class="hidden md:grid md:grid-cols-6 md:gap-2 md:mb-2 items-center">
            <div class="col-span-2">
              <a href={`/player/${sc.playerId}`}
                 class="text-blue-600 dark:text-blue-400 hover:underline">{sc.playerName}</a>
            </div>
            <div>{totalScore[i]}</div>
            <div>{onesCount[i]}</div>
            <div>{playedHolesCount[i]}</div>
            <div></div>
          </div>
        {/each}
      </div>
      ...
      {#if !session.submissionDateTime}
        {#if canSubmit}
          {#if !session.isFreeplay && moment().isBetween(moment(session.tournamentEndDateTime).subtract(31, 'minutes'), moment(session.tournamentEndDateTime))}
            ...
            <div
              class="my-4 p-3 bg-yellow-100 dark:bg-yellow-900 text-yellow-800 dark:text-yellow-200 rounded-lg flex items-center">
      ...
      {:else}
        <div class="text-green-600 dark:text-green-400 font-medium">
          ✅ {m.session_already_submitted({ submissionDate: moment(session.submissionDateTime).format(DATETIME_DISPLAY) })}</div>
      {/if}
    </TabItem>
      ...
      <TabItem title={sc.playerName}>
        <div class="grid grid-cols-4 gap-2 mt-4 dark:text-gray-300">
          {#each [...Array(session.holes).keys()] as index}
            ...
            <div>
              <Input class="w-15" type="number" max="7" min="0"
                     bind:value={sc.data[index]}
        ...
        <div class="mt-4 text-gray-700 dark:text-gray-300">
          <span
            class="font-semibold dark:text-white">{m.session_total()}</span> {totalScore[i]}
        </div>
        <div class="mt-4 text-gray-700 dark:text-gray-300">
          <span
            class="font-semibold dark:text-white">{m.session_ones_label()}</span> {onesCount[i]}
        </div>
      </TabItem>
    {/each}
    ...
    {#if showActionsTab}
      <TabItem title={m.session_actions()}>
        <div class="space-y-4 mt-4">
          <h2
            class="text-xl font-semibold dark:text-white">{m.session_available_actions()}</h2>

          {#if !session.submissionDateTime}
            <div class="space-y-2">
              <h3
                class="text-lg font-medium dark:text-white">{m.session_player_management()}</h3>
              <div class="flex flex-wrap gap-2">
                <PermissionGuard supabase={data.supabase}
                ...
                <PermissionGuard supabase={data.supabase}
                                 ...
                                 action={Action.Update}>
                  <Button type="button" color="blue"
                          onclick={() => showMovePlayerModal = true}>
                ...
                <PermissionGuard supabase={data.supabase}
                  ...
                  <Button type="button" color="red"
                          onclick={() => showRemovePlayerModal = true}>
          ...
          <div class="space-y-2">
            <h3
              class="text-lg font-medium dark:text-white">{m.session_danger_zone()}</h3>
            <div class="flex flex-wrap gap-2">
              <PermissionGuard supabase={data.supabase}
                               resource={Resource.Sessions}
  ...
  <Modal bind:open={showModal} autoclose size="sm">
    <div class="p-4 dark:bg-gray-800">
      <h2 class="text-lg font-bold mb-2 dark:text-white">
        ⚠️ {m.session_incomplete_scorecards()}</h2>
      <p
        class="text-sm mb-2 dark:text-gray-300">{m.session_incomplete_players()}</p>
  ...
  <Modal title={m.session_delete()} bind:open={showDeleteModal} size="sm">
    ...
    <div class="flex justify-end space-x-2">
      <Button color="light"
              onclick={() => showDeleteModal = false}>{m.session_cancel()}</Button>
      <Button type="button" color="red"
              onclick={() => { handleDelete(); showDeleteModal = false; }}>{m.session_delete_button()}</Button>
    </div>
  </Modal>
